...
class ImageFile(Dataset):
    def __init__(self, filename, downsample_factor=1):
        super().__init__()
        self.downsample_factor = downsample_factor
        self.img = Image.open(filename)
        self.img_channels = len(self.img.mode)
        self.size = self.img.size
        self.img_downsampled = self.img.resize((256,256))
        if downsample_factor > 1:
            size = tuple(map(lambda x: x//2, self.size))
            self.img_downsampled = self.img.resize(size, Image.ANTIALIAS)
...
class ImageFile_resize(Dataset):
    def __init__(self, filename,side_length, downsample_factor=1):
        super().__init__()
        self.downsample_factor = downsample_factor
        self.img = Image.open(filename)
        self.img_channels = len(self.img.mode)
        self.size = (side_length, side_length)
        self.img_downsampled = self.img.resize((side_length,side_length))
        if downsample_factor > 1:
            size = tuple(map(lambda x: x//2, self.size))
            self.img_downsampled = self.img.resize(size, Image.ANTIALIAS)
...
class ImageFile_resize_han(Dataset):
    def __init__(self, filename,side_length1,side_length2, downsample_factor=1):
        super().__init__()
        self.downsample_factor = downsample_factor
        self.img = Image.open(filename)
        self.img_channels = len(self.img.mode)
        self.size = (side_length2, side_length1)
        self.img_downsampled = self.img.resize((side_length2,side_length1))
        if downsample_factor > 1:
            size = tuple(map(lambda x: x//2, self.size))
            self.img_downsampled = self.img.resize(size, Image.ANTIALIAS)
...
class ImageFolder(Dataset):
    def __init__(self, folder_path, downsample_factor=1):
        super().__init__()
        self.downsample_factor = downsample_factor
        self.folder_path = folder_path
        self.image_files = [f for f in os.listdir(folder_path) if f.endswith(('png', 'jpg', 'jpeg'))]
        
        # Optional: Resize all images to 256x256 by default
        self.img_downsampled_list = []
        for img_file in self.image_files:
            img = Image.open(os.path.join(folder_path, img_file))
            self.img_channels = len(img.mode)
            if downsample_factor > 1:
                size = tuple(map(lambda x: x // downsample_factor, img.size))
                img_downsampled = img.resize(size, Image.ANTIALIAS)
            else:
                img_downsampled = img.resize((256, 256))
            self.img_downsampled_list.append(img_downsampled)
    ...
    def __getitem__(self, idx):
        if self.downsample_factor > 0:
            return self.img_downsampled_list[idx]
        else:
...
class ImplicitAudioWrapper(torch.utils.data.Dataset):
    def __init__(self, dataset, scale_coords=0):
        self.dataset = dataset
        self.grid = np.linspace(start=-100, stop=100, num=dataset.file_length)
        self.grid = self.grid.astype(np.float32)
        self.scale_coords = scale_coords
        self.grid = torch.Tensor(self.grid).view(-1, 1) * self.scale_coords if self.scale_coords != 0 else torch.Tensor(self.grid).view(-1, 1)
...
class Implicit2DWrapper(torch.utils.data.Dataset):
    # import pdb;pdb.set_trace()
    def __init__(self, dataset, sidelength=None, compute_diff=None , blur=None, scale_coords=0, scale_img=0):
        ...
        self.scale_coords = scale_coords 
        self.mgrid = get_mgrid(sidelength) * self.scale_coords if self.scale_coords != 0 else get_mgrid(sidelength) 
        self.transform = Compose([Resize(sidelength),ToTensor(),Normalize(torch.Tensor([0.5]), torch.Tensor([0.5]))])
        self.transform_scale = Compose([Resize(sidelength),ToTensor(),Normalize(torch.Tensor([0.5]), torch.Tensor([0.5/self.scale_img]) if self.scale_img != 0 else torch.Tensor([0.5]))])
        self.transform_unscale = Compose([Resize(sidelength),ToTensor(),Normalize(torch.Tensor([0]), torch.Tensor([1/255]))])
    ...
    def __getitem__(self, idx):
        ...
        if self.scale_img:
            img = self.transform_scale(self.dataset[idx]) ##origin0 
...
class Implicit2DWrapperBatch(torch.utils.data.Dataset):
    def __init__(self, dataset, sidelength=None, compute_diff=None, blur=None, scale_coords=0, scale_img=0):
        if isinstance(sidelength, int):
            sidelength = (sidelength, sidelength)
        self.sidelength = sidelength
    ...
    def __getitem__(self, idx):
        if isinstance(idx, int):
            idx = [idx]  # If idx is a single integer, convert to list
        ...
        for i in idx:
            img = self.transform(self.dataset[i])
            if self.scale_img:
                img = self.transform_scale(self.dataset[i])
            ...
            img = img.permute(1, 2, 0).view(-1, self.dataset.img_channels)
            in_dict.append({'idx': i, 'coords': self.mgrid})
            gt_dict_i = {'img': img}
            ...
            if self.compute_diff == 'gradients':
                gradients = torch.cat((torch.from_numpy(gradx).reshape(-1, 1),
                                       torch.from_numpy(grady).reshape(-1, 1)),
                                      dim=-1)
                gt_dict_i.update({'gradients': gradients})

            elif self.compute_diff == 'laplacian':
                gt_dict_i.update({'laplace': torch.from_numpy(laplace).view(-1, 1)})
