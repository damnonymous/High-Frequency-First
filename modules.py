...
from layers import WeightNormLayer, BN_self, LN_self, IN_self, GN_self,BGN_self

class CustomSine(nn.Module):
    def __init__(self, positive_scale=30, negative_frequency_multiplier= 2):
        super().__init__()
        self.positive_scale = positive_scale
        self.negative_frequency_multiplier = negative_frequency_multiplier

    def forward(self, input):
        positive_mask = input > 0
        # 正值部分：x * sin(positive_scale * x)
        positive_part = (1 + 5*torch.abs(input.detach()) )* torch.sin(self.positive_scale * input)
        # 负值部分：sin(negative_frequency_multiplier * input)，频率加倍
        negative_part = (1 + 5*torch.abs(input.detach()) )* torch.sin(self.positive_scale * input)
        # 结合正负部分
        return torch.where(positive_mask, positive_part, negative_part)


class SmoothSine(nn.Module):
    def __init__(self, frequency_multiplier=30):
        super(SmoothSine, self).__init__()
        self.frequency_multiplier = frequency_multiplier

    def forward(self, input):
        # Apply the sine function
        sin_input = torch.sin(self.frequency_multiplier * input)
        # Apply the square root to the absolute value of sin_input and then reapply the sign
        # Clamp min is used to avoid NaN gradients for negative values of sin_input
        smooth_sin_input = torch.sign(sin_input) * torch.sqrt(torch.abs(sin_input) + 1e-9)
        return smooth_sin_input
...
class BatchLinearNorm(nn.Linear, MetaModule):
    '''A linear meta-layer that can deal with batched weight matrices and biases, as for instance output by a
    hypernetwork.'''
    __doc__ = nn.Linear.__doc__
    def __init__(self, in_features, out_features, bias=True, norm_type=None):
        super(BatchLinearNorm, self).__init__(in_features, out_features, bias=bias)
    ...
    def forward(self, input, params=None):
        ...
        if self.norm_type is not None:
            if self.norm_type == 'BN':
                norm = BN_self(output.shape[2]).cuda()
            elif self.norm_type == 'LN':
                norm = LN_self(output.shape[2]).cuda()
            elif self.norm_type == 'GN':
                norm = GN_self(output.shape[2], groups=32).cuda()
            elif self.norm_type == 'BGN':
                norm = BGN_self(output.shape[2], groups=32).cuda()  ## default groups=64
            elif self.norm_type == 'IN':
                norm = IN_self(output.shape[2]).cuda()
            else:
                raise ValueError(f"Invalid norm type: {self.norm_type}. Please choose 'BN', 'LN', 'GN', or 'IN'.")
            output_norm = norm(output.squeeze(0)[:,:,None,None])
            output = output_norm.reshape(output.shape[0], output.shape[1], output.shape[2])
        else:
            output = output
        return output
...
class Sine(nn.Module):
    ...
    def forward(self, input):
        ...
        # import pdb;pdb.set_trace()
        # return (1+2*abs(input.detach()))*torch.sin(30 * input)  ##30
        # print('max',input.max())
        # print('min',input.min())
        return torch.sin(30 * input)  ##30
    
class Sine_han(nn.Module):
    def __init__(self,t):
        # import pdb;pdb.set_trace()
        super().__init__()
        self.weight_sin = t
    def forward(self, input):
        # See paper sec. 3.2, final paragraph, and supplement Sec. 1.5 for discussion of factor 30
        # import pdb;pdb.set_trace()
        return torch.sin(self.weight_sin * input)  ##30
...
class FCBlock(MetaModule):
    ...
    def __init__(self, in_features, out_features, num_hidden_layers, hidden_features,
                 outermost_linear=False, nonlinearity='relu', weight_init=None, norm_type=None, norm_layer_num=-1):
        super().__init__()
        ...
        nls_and_inits = {'sine':(Sine(), sine_init, first_layer_sine_init),
                        #'custom_sine': (CustomSine(positive_scale=30, negative_frequency_multiplier=30), sine_init_han, first_layer_sine_init_han), #custom_sine
                         'custom_sine': (CustomSine(positive_scale=30, negative_frequency_multiplier=30), sine_init, first_layer_sine_init), #custom_sine
        ...
        if norm_type is not None and norm_layer_num < 0:
            self.net.append(MetaSequential(
                BatchLinearNorm(in_features, hidden_features, norm_type=norm_type), nl
            ))
        else:
            self.net.append(MetaSequential(
        ...
        for i in range(num_hidden_layers):
            if i == norm_layer_num:
                self.net.append(MetaSequential(
                BatchLinearNorm(hidden_features, hidden_features, norm_type=norm_type), nl
                )) 
                continue
            self.net.append(MetaSequential(
...
class FCBlock_han(MetaModule):
    ...
    def __init__(self, in_features, out_features, num_hidden_layers, hidden_features,
        ...
        # import pdb;pdb.set_trace()
        self.net.append(MetaSequential(
            BatchLinear(in_features, hidden_features), Sine_han(55)
        ))
        # import pdb;pdb.set_trace()
        for i in range(num_hidden_layers):
            self.net.append(MetaSequential(
                BatchLinear(hidden_features, hidden_features),  Sine_han(58)
            ))
            if i ==2:
                self.net.append(MetaSequential(
                    BatchLinear(hidden_features, hidden_features),  Sine_han(65)
                ))
        ...
        if outermost_linear:
            ...
            import pdb;pdb.set_trace()
            self.net.append(MetaSequential(
                BatchLinear(hidden_features, out_features),  Sine_han(30)
            ))
...
class SingleBVPNet(MetaModule):
    ...
    def __init__(self, out_features=1, type='sine', in_features=2,
                 mode='mlp', hidden_features=256, num_hidden_layers=3, norm_type=None, norm_layer_num=-1, **kwargs):
        # import pdb;pdb.set_trace()
        super().__init__()
        self.mode = mode
        # import pdb;pdb.set_trace()
        ...
        self.net = FCBlock(in_features=in_features, out_features=out_features, num_hidden_layers=num_hidden_layers,
                           hidden_features=hidden_features, outermost_linear=True, nonlinearity=type, norm_type=norm_type, norm_layer_num=norm_layer_num)
        
        #han_start
        # self.new_param = nn.Parameter(torch.tensor(0.1))
        # self.han = nn.Sequential(
        #     nn.Linear(1, 16),
        #     nn.ReLU(),
        #     nn.Linear(16, 1),
        #     # nn.ReLU(),
        ...
        # )
        # self.han_e =torch.zeros(1, 1).cuda()#/500.0
        #han_end
        print(self)
    ...
    def forward(self, model_input, params=None):
        ...
        # print("Updated Linear Layer Bias:", self.net.net[4][0].bias.data) #self.net.net[4][0].bias.data
        output = self.net(coords, get_subdict(params, 'net'))
        
        # import pdb;pdb.set_trace()
        # self.new_param =self.han(self.han_e)#-0.3 #handongshen
        # with torch.no_grad():
        #     self.han_e = self.han_e +1
...
def first_layer_sine_init_han(m):
    if type(m) == BatchLinear or type(m) == nn.Linear:
        if hasattr(m, 'weight'):
            nn.init.xavier_normal_(m.weight)
